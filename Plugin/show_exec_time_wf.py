...
class ShowExecTimeWfCommand(sublime_plugin.WindowCommand):
    def run(self):
        ...
        regions = v.find_all(r'(Реализация определения процесса)|(Реализация определения шага)|(Остановка экземпляра шага)|(Остановка экземпляра процесса)')#, sublime.IGNORECASE,"\2:")
        for region in regions:
            for line in v.lines(region):
                ...
                s = s.replace('.', '')
                print (s)
                match = re.search('Реализация определения процесса', s)
                if match:
                    level = level + 1
                    ...
                    match = re.search('Реализация определения шага', s)
                    if match:
                        str2 = re.findall (r'\d{4}-\d{1,2}-\d{1,2}\s+\d{1,2}:\d{1,2}:\d{1,2}', s)
                        t1 = datetime.datetime.strptime(str2[0], "%Y-%m-%d %H:%M:%S")
                        ...
                        match = re.search('Остановка экземпляра шага', s)
                        if match:
                            str2 = re.findall (r'\d{4}-\d{1,2}-\d{1,2}\s+\d{1,2}:\d{1,2}:\d{1,2}', s)
                            t2 = datetime.datetime.strptime(str2[0], "%Y-%m-%d %H:%M:%S")
                            resultStr = resultStr + " [" + str((t2-t1).seconds) +' sec]\n'
                        else:
                            match = re.search('Остановка экземпляра процесса', s)
                            if match:
                                level = level - 1
                                str2 = re.findall (r'\d{4}-\d{1,2}-\d{1,2}\s+\d{1,2}:\d{1,2}:\d{1,2}', s)
                                t1 = datetime.datetime.strptime(str2[0], "%Y-%m-%d %H:%M:%S")
